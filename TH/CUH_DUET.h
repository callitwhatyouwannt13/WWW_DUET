...
/*Our algorithm CUH_DUET*/
/* the sketch consists of CU sketch + heap */
...
class CUH_DUET{
    private:
        ...
        BOBHash32 hashx;  
        int linenum[hL];
        unordered_map <string, int> est_TH;

        hbucket B[hD][hW];
        hcell C[hL][hR];

    public:
        CUH_DUET(int d, int w, int l, int r, int Nth, double Rth, int K): d(d), w(w), l(l), r(r), Nth(Nth), Rth(Rth), K(K){
            heap = new Heap(K);
            for (int i = 0; i < d; i++)
        ...
        void insert_filter(uint32_t x, uint32_t y){
            int pos[d];
            int min_x = INT_MAX;
            for (int i = 0; i < d; i++){
        ...
        void query_hottable(){
            for(int i = 0; i < l; i++){
                for(int j = 0; j < r; j++){
                    ...
                    int x_freq = heap->Query(C[i][j].cx);
                    if(x_freq > 0){
                        int xy_freq = C[i][j].cnt;
                        if (xy_freq >= Rth * x_freq){
