...
class SS_DUET{
    ...
    public:
        SS_DUET(int M1, int d, int w, int l, int r, int Xth, double Rth, int Nth): M1(M1), d(d), w(w), l(l), r(r), Xth(Xth), Rth(Rth), Nth(Nth) {
        ...
        void insert(uint32_t x, uint32_t y, string strx){
            ...
            if (freq < Nth){
                ...
                if (B[posy][posx].count == 0){
                    B[posy][posx].count = 1; B[posy][posx].x = x; B[posy][posx].y = y;
                }
                ...
                for (int i = 0; i < d ; i++){
                    ...
                    if (B[i][pos].x == x){
                        ...
                        B[i][pos].x = 0; B[i][pos].y = 0; B[i][pos].count = 0;
                    }
                }
            }
        ...
        int Insert_x(string x){
            ...
            return ss->sum[p];  //return the frequency
        }
        ...
        void insert_hottable(uint32_t x, uint32_t y, int f){
            ...
            if (flag_xy == 0){
                if (linenum[pos] < r){
                    C[pos][linenum[pos]].x = x; C[pos][linenum[pos]].y = y; C[pos][linenum[pos]].cnt = f;
                    linenum[pos]++;
                }
                    ...
                    if (C[pos][min_pos].cnt < 0){
                        C[pos][min_pos].x = x; C[pos][min_pos].y = y; C[pos][min_pos].cnt = -C[pos][min_pos].cnt;
                    }
                }
            }
        }
        ...
        void query_hottable(){
            for(int i = 0; i < l; i++){
                for(int j = 0; j < r; j++){
                    int x_freq = query_x(C[i][j].x); int xy_freq = C[i][j].cnt;
